...
object TransaksjonValidatorQuery {
        ...
        """
            ...
            WHERE K_TRANSAKSJON_S IS NULL
              AND INN_TRANSAKSJON_ID IN (SELECT INN_TRANSAKSJON_ID
                                         FROM T_INN_TRANSAKSJON
                                         WHERE (BELOPSTYPE IN ('01', '02', '04') AND NOT (day(COALESCE(DATO_FOM, '1900-01-02')) = 1 AND day(COALESCE(DATO_TOM, '1900-01-01') + 1 DAYS) = 1 AND month(DATO_FOM) = month(DATO_TOM)))
                                            OR (BELOPSTYPE IN ('03', '05') AND NOT (day(COALESCE(DATO_FOM, '1900-01-02')) = 1 AND day(COALESCE(DATO_TOM, '1900-01-31') + 1 DAYS) = 1)))
        """.trimIndent()
    ...
    val VALIDATOR_04_GYDLIG_BELOPSTYPE =
        """
            ...
            WHERE K_TRANSAKSJON_S IS NULL
              AND INN_TRANSAKSJON_ID IN (SELECT t1.INN_TRANSAKSJON_ID
                                         FROM T_INN_TRANSAKSJON t1
                                                  LEFT JOIN T_K_BELOP_T t2 ON t1.BELOPSTYPE = t2.K_BELOP_T
    ...
    val VALIDATOR_06_GYLDIG_REFERANSE =
        """
            UPDATE T_INN_TRANSAKSJON T_UPDATE
            SET T_UPDATE.K_TRANSAKSJON_S = '06'
            WHERE K_TRANSAKSJON_S IS NULL
              AND INN_TRANSAKSJON_ID IN (SELECT INN_TRANSAKSJON_ID
                                         FROM T_INN_TRANSAKSJON
                                         WHERE BELOPSTYPE BETWEEN '04' AND '05'
                                           AND REF_TRANS_ID = ''
                                         UNION
                                         SELECT INN_TRANSAKSJON_ID
                                         FROM (SELECT t1.INN_TRANSAKSJON_ID, COALESCE(t2.TRANS_EKS_ID_FK, t3.TRANS_ID_FK) AS REF_TRANS_ID
                                               FROM T_INN_TRANSAKSJON t1
                                                        LEFT JOIN T_TRANSAKSJON t2 ON t2.K_ANVISER = t1.AVSENDER AND t2.TRANS_EKS_ID_FK = t1.REF_TRANS_ID
                                                        LEFT JOIN T_INN_TRANSAKSJON t3 ON t3.AVSENDER = t1.AVSENDER AND t3.TRANS_ID_FK = t1.REF_TRANS_ID
                                               WHERE t1.REF_TRANS_ID != '') t4
                                         WHERE t4.REF_TRANS_ID IS NULL)
        """.trimIndent()
    ...
    val VALIDATOR_10_GYLDIG_BELOP =
        """
            ...
            WHERE K_TRANSAKSJON_S IS NULL
              AND (BELOP < 0 OR (BELOP = 0 AND BELOPSTYPE IN ('01', '02', '03')))
        """.trimIndent()
